...
export const Consultant = () => {
  ...
  const [strategy, setStrategy] = useState<DataStrategy | null>(null);
  ...
  return (
    <div className="max-w-3xl mx-auto py-20 px-6">
      <div className="text-center mb-16">
        ...
        <p className="text-[10px] opacity-50 uppercase tracking-[0.4em]">
          AI-Powered Architectural Audit
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          <div className="flex flex-col gap-3">
            <label className="text-[9px] uppercase tracking-[0.3em] font-medium opacity-40">
              Industry Sector
            </label>
            <select
              value={industry}
        ...
        >
          <span className="relative z-10">
            {loading ? 'ANALYZING INFRASTRUCTURE...' : 'GENERATE STRATEGIC BLUEPRINT'}
          </span>
      ...
      {strategy && (
        <div className="mt-24 space-y-16 border-t border-black/10 pt-16 animate-in fade-in slide-in-from-bottom-8 duration-1000 ease-out">
          <div className="max-w-2xl">
            <h3 className="text-[9px] uppercase tracking-[0.4em] opacity-40 mb-6 italic">
              — Strategic Summary
            </h3>
            <p className="text-2xl font-light italic leading-snug text-neutral-800">
              {strategy.summary}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-20">
            <div>
              <h3 className="text-[9px] uppercase tracking-[0.4em] opacity-40 mb-8 font-medium">
                High-Level Roadmap
              </h3>
              <ul className="space-y-6">
                {strategy.keyActionables.map((item: string, i: number) => (
                  <li key={i} className="text-sm font-light flex items-start gap-6 group">
                    <span className="text-[10px] opacity-20 font-mono mt-1 group-hover:opacity-100 transition-opacity">
                      0{i + 1}
                    </span>
                    <span className="leading-relaxed opacity-70 group-hover:opacity-100 transition-opacity">
                      {item}
                    </span>
                  </li>
                ))}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                {strategy.recommendedStack.map((tech: string, i: number) => (
                  <span
                    key={i}
                    className="px-4 py-2 bg-neutral-50 border border-black/5 text-[9px] uppercase tracking-[0.2em] font-medium text-neutral-600 hover:border-black transition-colors cursor-default"
                  >
                    {tech}
                  </span>
                ))}
